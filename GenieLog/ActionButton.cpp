...
/**
*\fn ActionButton::ActionButton(const string & label , Vector2f position , Vector2f size , Color bckgrnd , Color font-color , const Font & font)
* Cette fonction est le constructeur de la class ActionButton
	...
	:m_pushed(false), m_hoovered(false), m_base_font_color(font_color), m_base_main_color(bckgrnd)
{
	...
	/*! Le constructeur de ActionButton sert à construire le bouton c'est à dire , définir la taille de son texte ,sa couleur , la taille de son rectangle ,sa couleur également , etc.. */
}
/**
*\fn ActionButton::~ActionButton()
* Cette fonction est le destructeur de la class ActionButton
**/
...
/**
*\fn bool ActionButton::buttonPushed() const
*Cette fonction sert à savoir si on a appuyé sur un bouton
...
/**
*\fn void ActionButton::update(const Event & event) 
*Cette fonction gère les différents évenements effectués sur les boutons
*\param event Différents evenements possibles : Exemple clic-gauche - clic-droit etc .. C'est une fonction de librairie sfml.
**/
void ActionButton::update(const Event & event)
{
	...
	/*! Voici le fonctionnement de cette fonction :
		On initialise d'abord la variable global m_pushed à false pour signifier qu'aucun bouton n'a été actionner.
		Ensuite une série de condition s'en suit : - Si on clic sur un bouton on passe m_pushed à true.
							   - Si on passe avec notre souris sur un bouton , m_hovered est passé à true pour signifier qu'on passe dessus.
		Ensuite ces evenements testés , que faut-il faire si on passe dessus avec notre souris ? On change légèrement la couleur pour rendre l'interface un peu plus jolie.
		Sinon on remet la couleur du bouton par défaut!
*/
}
/**
*\fn void ActionButton::setState(bool state) 
*Cette fonction définit l'état du bouton , si on passe dessus , ou qu'on a cliqué ..
*\param state l'état du bouton
**/
void ActionButton::setState(bool state)
{
	m_pushed = state;//Fixe l'état du bouton 
	/*! Cette fonction fixe l'état du bouton */
}
/**
*\fn Action::draw(RenderTarget & target , Renderstates states) const 
*Cette fonction sert à afficher les boutons dans la fenetre SFML 
**/
